...
that is absorbing [transcribe.cpp](https://github.com/) per [`PLAN.md`](PLAN.md).

> **2026-09-26:** the plan documents cited in older entries below (`TO_DO_UNIFY_AND_IMPROVEMENT_PLAN_V6.md`,
> `FUSION_ROADMAP_PLAN.md`, `MULTI_AGENT_FUSION_PLAN_AND_TRACKER.md`, `progress.md`, several `docs/reports/*`)
> now live under [`docs/archive/`](docs/archive/). Ids they define are indexed in [`docs/LEDGER.md`](docs/LEDGER.md).
...
- **One plan.** `PLAN.md` v7 replaces the V6 plan, roadmap v6.0, tracker and progress.md, all now archived under `docs/archive/`.
  ...
  - Status is generated by `scripts/status.sh` (new, read-only).
  - `AGENTS.md` was rewritten, including the upstream sync rules formerly in the tracker.
- **Decisions V7-D1…D6 taken** (user, 2026-09-26):
  - `speech.h` is the only first-party ABI, and our `capi/` `audiocpp.h` will be retired, not shimmed;
  - CrispASR becomes reference parent #3, mined by family;
  - upstream-first;
  - the new task kinds;
  - the non-commercial model policy;
  - the process diet.
- **Checkpoint audit:** `docs/reports/checkpoint_2026-09-26.md`. It covers false DONE claims, unregistered tests, 10 % WER defaults, the two conflicting `audiocpp_*` ABIs, orphans, and the README describing the fusion inverted (that README section is now corrected).
- **Decision V7-D7 (user directives): two build profiles.**
  - **DEV** keeps everything (server + WebUI, CLI, tools, tests) to test every model.
  - **BUNDLE** is the embeddable library with only the selected families, down to one STT or TTS family, via model-set composites and Cargo features, like transcribe.cpp.
  - Primary consumers, both primary: FreeSpeech desktop (#1, successor of ZER0 and S2B2S) and FreeSpeech Android (#2).
  - PLAN §1, phase E (E0 composites, E6 Android), SC4 Multi-STT gate, AGENTS.md, README, strategy note and client build doc updated.
- **New `docs/glossary.md`:**
  - vocabulary for audio, ASR, TTS, streaming, runtime and speech.cpp terms;
  - how each metric is computed and read (WER/CER and normalization, RTF vs RTFx, latency, memory, MOS/UTMOS/SIM/round-trip WER, DER/VAD/LID, SDR/PESQ, BLEU/COMET, music F1, numeric parity);
  - the standard benchmarks per task, marking what speech.cpp uses.
  - Linked from README, AGENTS, PLAN, benchmarking.md and ARCHITECTURE.md.
- **Decision V7-D9 (user): benchmark acceptance.**
  - Ports must be no slower and no less accurate than both parents (FLEURS multilingual WER/CER, RTF, peak memory).
  - Runs: ≥ 3 with run 1 as the warm-up (reported, not averaged); mean of runs 2..N, with N = 3 / 5 / ≤ 10.
  - CPU while developing; CPU + CUDA + Vulkan-NVIDIA + Vulkan-Intel before acceptance.
  - New `docs/benchmarking.md`; PLAN rule R10, L12, 11b step 5, and C0.6 (port the author's transcribe.cpp bench/WER harness).
- **Decision V7-D10 (user): repomix architecture map + pre-commit habit.**
  - New generated `ARCHITECTURE.md` (`scripts/arch/gen-architecture.ts`, repomix 1.18.1 pinned).
  - `scripts/arch/repomix.ts` for scoped packs (gitignored `repomix-output.xml`).
  - `.githooks/pre-commit`, enabled locally with `core.hooksPath`: it regenerates and stages the map, then runs hygiene checks.
  - README, AGENTS and PLAN send every reader to the map first.
- **Decision V7-D8 (user): lineage and priority.**
  - The author's path is S2B2S → NairoDorian/transcribe.cpp (most of the work) → ZER0 → speech.cpp → FreeSpeech. audio.cpp is pure upstream.
  - speech.cpp absorbs all optimizations and models of both parents.
  - Engine ports must now match the arch they replace on RTF and peak memory (≤ 1.1×), not only WER. The cohere engine (+23 % RTF) is flagged.
- **Docs aligned with the directives:**
  - `PLAN.md` gains a canonical **North star** (the end goal + rules R1–R9) that every doc points to.
  - The README header, highlights, transcribe.cpp families section, bindings, tests, project layout and license are rewritten to match the tree. Fixed: the license is Apache-2.0 (was mis-stated as MIT); 20 dead model-card links now point to transcribe.cpp; no longer claims bindings or paths that don't exist.
  - Speech.cpp notes were added to the transcribe.cpp-derived porting and testing guides (`src/arch/` → engine packages, rule R5), to `docs/bindings.md` and `capi/README.md` (legacy ABI retiring), to the WER-gate report (10 % bound vs S1.3) and to the rethink note (partly superseded).
  - The transcribe.cpp triage ledger gets a re-triage note for its Rust / ARCH_DL rows.
- **Strategy proposals SC1–SC7** (pending the user): `docs/reports/strategy_freespeech_2026-09-26.md` and PLAN §8. Real-time conversation runtime, resource manager, catalogue tiers and scoreboard, with FreeSpeech as the primary consumer.
- **CrispASR analysis:** `docs/reports/crispasr/`, `docs/upstream/crispasr_triage.md` (watermark `dfcdacae`).
- **Fixed: model-backed ASR tests were far too slow.**
  - Verdict and parity tests now default to one 3.5 s clip (`assets/asr_validation/quick/`, label `quick`; verdicts `--max-clips 1 --no-batch`; voxtral parity `--quick`).
  - Multi-clip `*_full` variants register only with `-DSPEECHCPP_FULL_ASR_TESTS=ON`.
  - Verdicts register only when their engine family is linked.
  - Parity tests run whichever fixtures exist instead of skipping.
  - Voxtral parity's engine side uses the same thread default as the C ABI.
  - Quick tier: 19 tests in 4.4 min (was >35 min, unfinished).

### In verification (2026-09-24, development batch — written, not yet built/tested)

Coded in parallel (lead + port agents) per the session's "code now, test at the end" direction; each item moves
to Fixed / Changed / Added once the end-of-session CPU build + suites + verdict runs pass.

- **C-ABI adapter ran every engine family single-threaded** when `n_threads = 0` ("library default"): now
  `default_n_threads()` like the arches (cohere engine RTF 1.05 -> 0.455, measured before the batch).
- **Family registry rejected transcribe-layout GGUFs for engine families**: canonical ids are now the engine
  packages (`canary_asr`, `cohere_asr`, `moss_transcribe_diarize`) with the arch names as aliases, and the loader
  lookup falls back to aliases / GGUF archs. The dead adapter row `"moss"` is removed.
- **ASR model composite linked MOSS-TTS instead of the MOSS ASR model** and omitted `cohere_asr`, `canary_asr`,
  `confucius4_r2t2`, `granite5asr`, `nemotron_3_diar`; fixed.
- **TokenizerHub** decodes SentencePiece-marked GGUF `bpe` vocabularies (parakeet / gigaam / medasr) by piece join
  with `<0xNN>` byte fallback instead of GPT-2 byte decoding (literal U+2581 leaked into text).
- **MelExtractor** `PadMode::None` on the non-power-of-two FFT path framed the signal half an FFT late.
- **Engine Whisper** decodes with one cached single-token step graph per KV cache (was a graph build + allocation
  per token); `SPEECHCPP_WHISPER_STATIC_STEP=0` restores the old path.
- **Framework result surface**: `TaskResult::token_timestamps` + `TimestampGranularity::Token`, `raw_text`,
  per-item batch status (`item_status`), `CapabilitySet::max_audio_ms`; the adapter maps them to TOKEN rows,
  `transcribe_get_raw_text`, per-utterance batch statuses (a batch the family rejects as a whole is re-run per
  utterance) and `max_audio_ms`, and enforces a non-AUTO timestamps request as a ceiling for every engine family.
- **Engine packages reading transcribe.cpp GGUFs**: `cohere_asr`, `canary_asr`, `moss_transcribe_diarize`
  (dual layout via `SpecBackedVoiceModelConfig::accepts_foreign_layout`); new ports `medasr`, `gigaam`,
  `canary_qwen` (integrated), `voxtral`, `granite_speech`, `granite_nar`, `parakeet_tdt` dual layout (in flight).
- **Verdict tooling**: `SPEECHCPP_ENGINE_ARCHS` routes listed GGUF archs to the engine while the builtin arch
  exists; `abi_arch_engine_verdict` (`ctest -L verdict`) diffs every C-ABI result field arch vs engine on the
  same file. Nine more test GGUFs pinned (revision + sha256) in `scripts/fetch_asr_test_model.py` (+ `HF_TOKEN`).
- `engine::assets::resolve_bundled_asset` - the default Silero VAD path no longer depends on the working directory
  (sense_asr / granite5asr / C-ABI VAD failed with ERR_BACKEND outside the repo root).
- Registry + adapter rows for `confucius4_r2t2` and `nemotron_3_diar`.
