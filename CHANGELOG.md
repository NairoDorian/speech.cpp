...
- **Phase 10.5, family 3 of 5 (step 2): the chunked Sortformer scheduler and the NeMo package layout, in the engine (2026-08-27)** — full report: `docs/reports/sortformer_diar_engine_port.md`.
  ...
  - **The AOSC speaker-cache + FIFO scheduler is ported** (`src/models/sortformer_diar/streaming.{h,cpp}`, host-for-host from `arch/sortformer/stream.cpp`: `streaming_update`, `_get_silence_profile`, the `_compress_spkcache` stack, NeMo's chunk windows, the preset table, the option/env precedence). The whole-window graph is split into a **stem graph** and a **body graph** built per 64-frame capacity tier and kept in a small LRU — the arch rebuilt both graphs for every chunk, which is why its `low_latency` ran below realtime. A package that ships a streaming operating point runs **chunked by default** (what the arch shipped); the HF v1 checkpoints keep the whole-window graph. Request/session options `stream_preset` (`default` / `offline` / `very_high_latency` / `high_latency` / `low_latency` / `small`) and per-field `stream_chunk_len` / `stream_left_context` / `stream_right_context` / `stream_fifo_len` / `stream_spkcache_len` / `stream_update_period`; `TRANSCRIBE_SORTFORMER_STREAM_*` env stay as validation hooks. One progress unit per chunk; `RunControl` unwinds between chunks. `SortformerDiarSession::last_probabilities()` exposes the model's actual output for parity tools.
  - **The typed `transcribe_sortformer_stream_ext` (RUN slot) survives the retirement**: the ArchAdapter accepts it, validates it pre-clear (a wrong kind or preset 99 leaves the previous speaker segments intact) and translates it into `stream_preset`. Gate `sortformer_diar_ext_abi_test` (custom tree, v1 package until the builtin `sortformer` arch is retired): probe, init, pre-clear rejection, ext `VERY_HIGH_LATENCY` == env `very_high_latency` rows, `LOW_LATENCY` runs — **OK**.
  - **Gates.** `sortformer_diar_streaming_engine_test` on the pinned v2 package: single-speaker fixtures diarize as one speaker chunked by default; the committed 2-speaker oracle mix separates both speakers under every operating point (DER 0.31–0.34 against the golden RTTM — **the arch's own probabilities score 0.315–0.334 on this synthetic clip**, so the bound is parity with the arch, not absolute quality); **chunked == whole-window to 1.8e-7** on a single-chunk clip; determinism; cancellation between chunks; unknown preset refused; geometry override applied. `sortformer_diar_scheduler_test` covers the host logic without a model. Suite **111/111** on `build-cpu-core`.
  - **Parity with the arch on identical weights** (the pinned package re-keyed into the arch's layout, tensor bytes untouched): max |Δp| 7.4e-3, mean 1.7e-4, **0 of 600 speaker decisions flipped** on the three published operating points; `low_latency` / `small` (the compression-heavy geometries) flip 1/600. Decision parity, not tensor parity to the arch's 4.5e-4 NeMo tolerance; the residual is op ordering in the engine's own modules, not weight storage (native Q8_0 gives the same size) — a Track M `validate.py` item, recorded in the report.
  - **Fixed on the way (1/4) — every strictly spec-validated family was unusable through the C ABI.** `apply_run_params` forwards `language`, `task`, `pnc`, … as request options; `sortformer_diar`, `sense_asr`, `hviske_asr` and `higgs_audio_stt` validate request options against their contract and rejected the first one (`unknown request option: language`). The adapter now prunes to the family's contract.
  - **Fixed on the way (2/4) — a package's embedded spec outranked the build's.** An audio.cpp GGUF resolved its contract from the spec embedded at conversion time, so an option added later to `model_specs/<family>.json` (`stream_preset`; and step 1's `cancellation` capability, which never actually reached the v1 package) was rejected by the session that implements it. The build's copy (workspace, else builtin) now wins when present; the embedded copy remains the fallback for a catalog-less runtime.
  - **Fixed on the way (3/4) — a GGUF without an embedded spec was refused a contract outright**, a rule written for audio.cpp's converter output and applied to every GGUF. Packages whose `general.architecture` is not `audiocpp` now resolve their contract like a safetensors checkout (`minimax_h3` already had a private carve-out).
  - **Fixed on the way (4/5) — the schema validator had no `cancellation` capability**, so the workspace spec step 1 edited was invalid; unnoticed because the v1 path never validated it (2/5). It is now a cross-task capability.
  - **Fixed on the way (5/5) — registry auto-detection decoded a foreign GGUF as Silero VAD.** `bf0e68a` closed the `can_load()`-order mis-detection for audio.cpp packages by reading their embedded family; a GGUF written by another tool carries no such key, so `audiocpp_cli --model diar_streaming_sortformer_4spk-v2.q8_0.gguf` without `--family` still failed with `missing tensor: stft_conv.weight`. `ModelRegistry::find_loader` now resolves a non-`audiocpp` `general.architecture` through `family_registry` (the §5.5 map that exists for exactly this) before probing, and says so when the mapped family is not linked. Verified: the v2 package auto-detects on the CLI (chunked, `low_latency` RTF 0.42 on the oracle), v1 still does.
  - **What step 3 (the deletion) has to answer for**, found by reading the consumers: the parakeet multitalker arch **includes** the sortformer arch's embedded-diarizer surface (`SortformerEmbedded`, `init_embedded_diarizer`, `run_diar_streaming_core`), and the parakeet arch is canonical (bake-off) — so the standalone family (its dispatcher entry, `Arch` hooks, and the public ext initializer's move to `transcribe-family-ext.cpp`) can retire, while the embedded core stays with parakeet until that family migrates. Retiring the dispatcher entry is also what routes the NeMo v2 package to the engine through the C ABI.

- **Phase 10.5, family 3 of 5 (step 1): `sortformer_diar` gets its first registered gate, and cancellation (2026-08-26)**:
  - The family had **no registered test at all** — only a warm-bench binary that is built off by default. `sortformer_diar_engine_smoke_test` now diarizes the four vendored LibriSpeech fixtures through the public engine surface and holds turn ordering, speaker count, speech coverage, determinism across runs, and cancellation. Measured on the v1 package: 1 speaker per fixture, coverage 0.86–1.03, 0.18–0.85 s per fixture against 2.1–14.2 s of audio.
  - **Cancellation merged.** The arch advertises `TRANSCRIBE_FEATURE_CANCELLATION`; the engine honoured `request_abort()` nowhere. The session polls `RunControl` at its three stage boundaries (there is no decode loop in a whole-window diarizer, so an abort unwinds within a stage — the capability says exactly that). Spec-backed families declare capabilities in their model spec, so `"cancellation"` is now a capability string read by `capabilities_from_spec`; absent means false, so no other family's contract changes.
  - **Two findings for the port that follows**: the catalogue's *default* v2 package is a **transcribe.cpp-flavoured GGUF** (`general.architecture` = `sortformer`) while v1 is an audio.cpp package — so unlike families 1 and 2, this arch can load a published package and the engine cannot load that one. And the arch's "streaming presets" are a **scheduler**, not a knob: chunked diarization with an AOSC speaker cache and FIFO, 475 LOC of host logic plus a two-graph split, against an engine package that is offline-only with a single whole-window graph.

- **Phase 10.5, family 2 of 5: the `voxtral_realtime` verdict executed — arch retired, typed extension preserved (2026-08-26)**:
  - **`src/runtime/arch/voxtral_realtime/` is deleted** (4,996 LOC; ledger B12, revert `fdaa9a5`), along with a never-registered smoke test. `src/models/voxtral_realtime/` is the single implementation.
  - **`num_delay_tokens` is a caller knob again.** The engine had it hardwired in four places — the offline right-pad, both first-chunk geometry helpers, and the prompt (as a literal `0.480` seconds). It is now one validated value (`1..15` or `30`, the publisher's set) threaded through all of them, fixed for the life of a stream, settable as a session option (`voxtral_realtime.num_delay_tokens`) or a request option (`num_delay_tokens`).
  - **`min_decode_interval_ms`** arrives as a partial-emission throttle, with the difference stated in the code: the arch needed it because its partial decode reprocessed the accumulated buffer; the engine decodes incrementally, so it bounds how often the delta is published — never how much is decoded, and never the final transcript.
  - **Cancellation** (`RunControl` per decode step, `CapabilitySet::supports_cancellation`), and the adapter now maps a mid-stream cancel to `TRANSCRIBE_ERR_ABORTED` instead of a generic backend error.
  - **The C ABI keeps the family's typed stream extension.** The ArchAdapter validates it pre-clear with the arch's exact rules and translates it into request options; `transcribe_model_accepts_ext_kind` answers for it. The public initializer `transcribe_voxtral_realtime_stream_ext_init`, which had been defined *inside the arch*, moved to `src/runtime/transcribe-family-ext.cpp` — deleting an arch must not delete a published symbol.
  - **Fifth pinned model**: `voxtral-mini-4b-realtime-2602-q4_k.gguf` (3,097,662,432 bytes, sha256 `8cafef18…`) — the smallest catalogued package for this 4B family. New gates `voxtral_realtime_engine_smoke_test` (**corpus WER 2.89855%, 2/69**, streamed-vs-offline divergence 0), `voxtral_realtime_delay_test`, and `voxtral_realtime_ext_abi_test` through the public C ABI.
  - **Fixed — streaming silently dropped the tail of a stream (L11).** `finalize()` ran the same loop as a feed, which stops when the remaining audio is shorter than one chunk, so a caller who stopped feeding mid-chunk lost the last word with no flag and no error (measured: streamed `" I'm from the cutter lying off the"` vs offline `" I'm from the cutter lying off the coast."`). It now zero-extends and decodes the remainder. transcribe.cpp's arch never had this bug — its `stream_finalize` re-ran the offline forward. This is the second family where the engine truncated silently and the arch did not.
  - **Fixed — the C ABI streaming path was unusable for every framework ASR family.** The ArchAdapter prepared a stream with no audio contract (a stream has no audio yet) and ASR sessions reject that; its comment claimed families default to 16 kHz, which they do not. It now supplies the contract the C ABI guarantees. The existing streaming C-ABI tests missed it because they run a VAD model or a GGUF that still resolved to a builtin arch.
  - **Fixed — the first streaming chunk ignored a non-default `num_delay_tokens`**, so the encoder produced a different audio-token count than the prompt and the decoder refused the stream. Exposed only because the new C ABI test asks for delay 4 rather than the default 6.
  - **Fixed on the way**: the batched path passed the transcription delay to the frontend but not to the prompt, so a non-default delay would have handed the decoder the wrong token budget; and `stat()` overflows above 2 GB on MSVC, which made a gate report SKIP on a model that was present.

- **Phase 10.5, family 1 of 5: the `qwen3_asr` verdict executed — arch retired, features merged (2026-08-26)**:
  - **`src/runtime/arch/qwen3_asr/` is deleted** (4,329 LOC; ledger B11, revert `9cc5457`) together with six `tests/transcribe/qwen3_asr_*.cpp` that had been copied from transcribe.cpp but never registered anywhere. `src/models/qwen3_asr/` is the single implementation.
  - **Speculative decode merged.** transcribe.cpp's 1-gram-lookup mechanism is now the engine's: `NgramLookupDrafter` (family-agnostic, in `modules/transformers/causal_lm_ops`) plus a verify graph built on a **new shared primitive** — `QwenDecoderLayerModule::build_with_static_cache_tail` accepts `steps ≥ 1` and `FastKVSetRows` gained a multi-row single-sequence mode — so no decoder math was duplicated and Phase 11a's shared driver can lift it as is. Request option `spec_k_drafts` follows the transcribe convention (−1 default/0 off/1..8); `CapabilitySet::supports_speculative_decode` is mapped to `transcribe_capabilities::supports_spec_decode` by the C ABI adapter. Default stays 0: on the four LibriSpeech fixtures k=4 reproduces k=0 **exactly** (divergence 0 words) but costs RTF 1.23 vs 0.81, as transcribe.cpp documented for short clips.
  - **HF BPE parity ported** (`qwen3_asr_bpe_parity_test`, same fixture file, 37 strings + 30 language prefixes exact), **BCP-47 language hints** (`languages.h`; `build_prompt` accepts "en" and "English"), and **cancellation** (`RunControl` polled at every decode step; `CapabilitySet::supports_cancellation`; the adapter bridges `transcribe_set_abort_callback` onto the progress callback and returns `TRANSCRIBE_ERR_ABORTED`, advertising `TRANSCRIBE_FEATURE_CANCELLATION` for engine families that poll).
  - **Fourth pinned model**: `qwen3-asr-0.6b-q8_0.gguf` from `audio-cpp/audio.cpp-gguf` (1,151,272,416 bytes, sha256 `6c44ec2f…`). New gates `qwen3_asr_engine_smoke_test` (**corpus WER 2.89855%, 2/69**, held to that baseline), `qwen3_asr_bpe_parity_test`, `ngram_lookup_drafter_test`, and `asr_e2e_qwen3_asr_wer_test` through the public C ABI (**corpus WER 2.89855%, 2/69, RTF 0.763**, in a product tree that links the family).

  - **Fixed on the way (1/3) — the C ABI could not open any audio.cpp GGUF.** `transcribe_model_load_file` consulted the framework registry only for non-GGUF paths, and audio.cpp packages carry `general.architecture = "audiocpp"` (family in `audiocpp.model_spec.family`), so every one of them returned `TRANSCRIBE_ERR_UNSUPPORTED_ARCH` (measured: `status=5`). They now route through the adapter like a safetensors directory.
  - **Fixed on the way (2/3) — model auto-detection could pick the wrong family** (`bf0e68a`). `ModelRegistry::find_loader` took the first loader whose `can_load()` accepted the path, and several loaders accept any GGUF whose resource bundle resolves (Silero VAD among them), so an audio.cpp GGUF resolved by registration order: the Qwen3-ASR file loaded as Silero VAD and failed at run time with `missing tensor: stft_conv.weight`. Auto-detection now reads the file's own `audiocpp.model_spec.family`. Applies to `audiocpp_cli --model`, the server and the C ABI alike.
  - **Fixed on the way (3/3) — batched decode was broken for every Qwen-decoder family** (`qwen3_asr`, `higgs_audio_stt`, `vibevoice_asr`): `build_with_static_cache_tail_batched` rotated `[n_seqs, 1, heads, dim]` heads with `[n_seqs]` positions, which RoPE (and `ggml_rope_ext`) rejects for any batch ≥ 2 — `positions shape mismatch: expected [1], got [2]`. The heads are now rotated through a `[1, n_seqs, heads, dim]` view. Identical code upstream; a candidate audio.cpp contribution.

- **Fusion review from the parents' code, and roadmap v6.0 (2026-08-26)**:
  - `docs/reports/fusion_review_2026-08-26.md` re-derives the merge plan from `audio.cpp` and `transcribe.cpp` source first, then compares it against the tracking docs. Both parents are treated as equally authoritative (§9.0 / L13).
  - **What the review found**: Phase 10 delivered the fused modules but **none of its five loser-feature merges or arch deletions were executed** (the deletion ledger has never been exercised); the three engine ports (W1a, W1b, W2a) each carry a **private KV cache and decode loop** — the child now has 8 enc-dec KV caches and 15 hand-rolled argmax loops; `transcribe-vad*` is a verbatim copy of what `engine/audio/chunking` already does; the Whisper encoder exists three times (arch, package, upstream `WhisperEmbeddingModule`); the W2a package **silently drops audio past 30 s** (`transcription.truncated` never reaches `TaskResult`); `capi/audiocpp.h` became a third façade with no `struct_size` and no consumer outside the tests; both C-ABI options ship OFF.
  - **What changed in the plan** (`FUSION_ROADMAP_PLAN.md` v6.0, tracker, `progress.md`, V6 R14): new laws **L11** (never truncate silently), **L12** (measure the real flow), **L13** (two parents, one child — `scripts/sync-deps.sh` at every phase boundary); new **Phase 10.5** (execute the Phase-10 verdicts, first deletions B11–B15); new **Phase 11a** (an ASR runtime layer — `EncDecKVCache`, decode drivers, `AsrResult`/`AsrLimits`, long-form over `audio/chunking` — and the re-basing of the three existing ports onto it, folding Whisper onto the upstream encoder and making the `.bin` loader a `TensorSource`); **Phase 12 pulled forward** (`speech.h` thin over the engine; `audiocpp.h` frozen, no new entry points — F14); **Phase 11c deletes `src/runtime/` in full**, reversing v5's "keep the dispatcher"; **Track M** carries transcribe.cpp's goldens/tolerances/`validate.py` methodology onto audio.cpp's own families on a per-phase quota. V6 decisions D2/D3/D4/D14/D15/D18/D19/D23 are recorded as superseded (roadmap Appendix F).
  - No code changed in this increment; the W2a truncation defect is recorded and gated (A21) and is fixed when `AsrLimits` lands in 11a.3, not patched locally.

- **Phase 11 Wave W2a Completed: Native Engine Whisper Package (`src/models/whisper/`)**:
  - **Third family migrated off the transcribe arch layer.** Whisper resolves through `ModelRegistry` by canonical id (`whisper`) or alias (`whisper-offline`), so CLI, server, WebUI and the shipped `audiocpp` C ABI can load it directly.
  - Numerics-identical port of the encoder (2-layer Conv1d stem → GELU(erf), learned absolute positional embedding, pre-LN blocks, final LN) and decoder (token + learned positional embedding with **no post-embed LayerNorm**, pre-LN blocks, logits head **tied** to the token embedding). The Whisper attention quirk is carried verbatim: **q/v/out carry a bias, k does not** — self- and cross-attention alike. The two conformer helpers the arch borrowed are reimplemented locally, so the package has **no `src/runtime/` dependency**.
  - **The frontend is reuse, not a port**: the Phase-9 `engine::audio::MelExtractor` runs Whisper's `PerUtterance` normalization over the exact slaney filterbank shipped inside the `.bin`. That is the "unified MelExtractor" half of W2's scope, delivered without a second private mel implementation.
  - **Weights load from the legacy whisper.cpp `.bin`**, not a GGUF, because `model_specs/whisper.json` is catalog-only (its 16 packages point at `Whisper-*-GGUF` paths absent from `audio-cpp/audio.cpp-gguf`). `assets.cpp` carries a metadata-only parser plus the special-token and suppress-list synthesis whisper.cpp derives rather than stores; payloads stream per tensor at upload time so a 1.5 GB large-v3 `.bin` is never held twice. `can_load()` sniffs the `ggml` magic rather than resolving a spec bundle, which would fail for every real model.
  - **A real bug the gate caught, worth remembering**: `MelExtractor` writes **mel-major** (element (m,t) at `m*n_frames + t`) while a graph input declared `ggml_new_tensor_2d(ctx, F32, n_mels, n_frames)` has `n_mels` as its **fastest** axis and therefore needs **frame-major**. Passing the extractor's buffer straight through fed the encoder a **transposed spectrogram**. Nothing crashed, the mel statistics looked healthy, and the decoder emitted confident fluent English unrelated to the audio ("You", "So", "I'm going to go to the next one.") at **94% WER**. Only the end-to-end numeric gate surfaced it. **Any family wiring MelExtractor into a ggml graph must transpose.**
  - **Gate `whisper_engine_smoke_test`** is held to **parity with the arch**, not the 10% structural bound: it fails if the engine path exceeds the 3 word edits `asr_e2e_whisper_wer_test` measures. **Measured: corpus WER 4.34783% (3/69), RTF 0.155 — exactly the arch baseline**, matching how W1a (1/69) and W1b (3/69, divergence 0) landed.
  - Suite: **103/103 green**. Arch copy untouched per §4.4 coexistence.
  - **W2b remains**: temperature-fallback ladder + DecodeTelemetry, timestamps, long-form seek continuation, language detection for multilingual variants, batched decode. W2a decodes one 30 s window greedily and marks longer audio truncated.
...
- **Phase 11a B31 Part 1: WhisperBinTensorSource**: added `framework/assets/whisper_bin.{h,cpp}` implementing a shared `TensorSource` for the legacy Whisper `.bin` weight format. Registered in `open_tensor_source()` so both arch and engine paths load `.bin` weights through the standard `TensorSource` interface, eliminating the private `load_whisper_assets()` parser.
