...
| **Phase 11** | Architectural family migration (Waves W1–W6) | `[~] IN PROGRESS — W1a + W1b DONE` | 18 families migrated to unified engine |
...
- **Current Timestamp**: 2026-08-26 (Upstream reconciliation to `c79e588` — **0 behind**; dual-parentage doctrine recorded; ggml bumped to `36da5713` / v0.22.0 and certified CPU **and** CUDA; **Phase 11 Wave W1 complete** (W1a + W1b); **W2 verification asset secured** — Whisper arch baseline locked at 4.34783%)
...
- **IMMEDIATE NEXT TASK FOR AGENT**:
  - **Wave W1 is complete** (W1a + W1b both green, both reproducing arch numerics). Two ways forward — take them in this order:
  - **(a) Wave W1 retirement step** (gated, needs user review): delete `src/runtime/arch/{moonshine,moonshine_streaming}/` as their own commits with Appendix B rows B16a/B16b. The ledger evidence is in hand: `moonshine_engine_smoke_test` (1/69 edits == arch) and `moonshine_streaming_engine_smoke_test` (3/69 offline == 3/69 streamed == arch, divergence 0).
  - **(b) Execute Phase 11 Wave W2 (Whisper Universal Family)** — **the verification asset is now in place**: `ggml-tiny.en.bin` is pinned and `asr_e2e_whisper_wer_test` locks the arch baseline at **corpus WER 4.34783% (3/69), RTF 0.047**. Port `src/runtime/arch/whisper/` (7,231 lines) to `src/models/whisper/` and hold the engine package to that number, exactly as W1a/W1b were. Scope: tiny…large-v3-turbo + `.en` variants, legacy `.bin` loader, suppress tables, temperature-fallback ladder + DecodeTelemetry, unified MelExtractor + TokenizerHub.
    - **Check `whisper.json`'s gguf source against the actual pinned GGUF before porting — but scope it correctly.** A `files:` block on a gguf source is the **norm and is correct**: `audiocpp_gguf` embeds sidecars by default and fails conversion when it cannot find them (`--no-sidecars` is the explicit opt-out), so 54+ catalog specs legitimately declare one. The defect only hits families pinned to **third-party GGUFs this pipeline did not produce** — which is what bit `moonshine.json` (W1a) and `moonshine_streaming.json` (W1b), both pulling from `handy-computer/moonshine-*-gguf` and both reporting `embedded_sidecars=false`. Decide per family with `audiocpp_gguf --inspect <gguf>`: `false` means drop the `files` block, `true` means leave the spec alone. Do **not** rewrite `files` blocks across the catalog on the pattern alone — for in-house GGUFs that would break working specs.
    - Note for any streaming family: `StreamingSessionBase` guarantees **append-only** committed text, NOT that `committed_text()` remains a live prefix of `full_text()` — a from-BOS re-decode may revise an already-committed region and the base keeps the old commit. Assert the append-only contract in gates.
