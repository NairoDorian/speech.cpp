...
| **Phase 10.5** | **Execute the Phase-10 verdicts** — 5 feature-merges, 5 arch deletions, ledger rows *(roadmap v6.0)* | `[~] IN PROGRESS — qwen3_asr (B11) + voxtral_realtime (B12 fdaa9a5) DONE; sortformer_diar: feature-merge DONE (step 2, 2026-08-27: chunked AOSC/FIFO scheduler + presets + typed ext + the NeMo package layout in the engine; 111/111), retirement (step 3) next — scoped to the standalone family because the parakeet arch consumes the embedded-diarizer core` | B11–B15 revert commits filled; no shadowed GGUF arch |
