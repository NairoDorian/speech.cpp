...
- [ ] **Wave W2 — Whisper Universal Family** (`whisper`: tiny…large-v3-turbo, .en variants, legacy `.bin` loader, suppress tables, temperature-fallback ladder + DecodeTelemetry; unified MelExtractor + TokenizerHub).
  ...
  - [ ] **W2b — remaining Whisper scope**: temperature-fallback ladder + DecodeTelemetry, timestamps, long-form seek continuation, language detection for multilingual variants, batched decode, and the static-topology step graph (W2a rebuilds the step graph per token, which is why its RTF is 0.155 vs the arch's 0.047 — a dispatch cost, not a numerics difference).
...
- **IMMEDIATE NEXT TASK FOR AGENT** *(roadmap v6.0, 2026-08-26)*:
  ...
  - **Phase 10.5 COMPLETE.** All 5 overlapping families have been retired: qwen3_asr (B11, 9cc5457), voxtral_realtime (B12, fdaa9a5), sortformer_diar step 3 (B15, a8b7a03b), sense_asr (B13, e3e7eac1), and fun_asr_nano (B14, 1be9ac40). The `fun_asr_nano` retirement was the last action: deleted the arch dir (~3,372 LOC), dropped it from the `transcribe-arch.cpp` dispatch table, removed the orphaned `transcribe-kaldi-fbank.cpp` from the runtime build, added the `asr_e2e_fun_asr_nano_wer_test` gate, and verified via `cpu-core` (engine_transcribe_runtime, test_adapter_sniff_dispatch compile + test_adapter_sniff_dispatch passes). Note: `cpu-full` has a **pre-existing** `firered_audio/qwen35_runtime.cpp` ggml API mismatch from the upstream `3eccab50` merge that is unrelated to this work.
  - **Execute Phase 11a** (next): ASR runtime shared layer — `EncDecKVCache` (`framework/asr/`), decode drivers (`DecodeDriver` / greedy + speculative), `AsrResult`/`AsrLimits` (fixes W2a's 30 s truncation, L11), long-form over `audio/chunking`, the three existing ports (W1a/W1b/W2a) re-based onto shared KV cache + decode driver (B30, eliminating 8 private caches + 15 argmax loops), Whisper folded onto `WhisperEmbeddingModule` + `.bin` as `TensorSource`, and `transcribe-vad*` deletion (B29). **No new ASR family port before 11a exists.**
