...
| **Phase 11a** | **ASR runtime layer** — `EncDecKVCache`, decode drivers, `AsrResult`/`AsrLimits`, long-form over `audio/chunking`; re-base W1a/W1b/W2a; fold Whisper onto `WhisperEmbeddingModule`; `.bin` as `TensorSource`; delete `transcribe-vad*` *(v6.0)* | `[~] FOUNDATION` | B29 VAD dedup done (c59b15a0); `AsrResult`/`AsrLimits` created; `TaskResult.truncated` added + propagated through all 3 sessions (L11/W2a fix). `EncDecKVCache` + `DecodeDriver` + 3-port re-base + WhisperEmbeddingModule + transcribe-vad* remain. |
...
- **IMMEDIATE NEXT TASK FOR AGENT** *(roadmap v6.0)*:
  - **Phase 11a continuation**: build `EncDecKVCache` + `DecodeDriver` in `framework/asr/`, re-base W1a/W1b/W2a onto the shared layer (B30: 8 private KV caches + 15 argmax loops eliminated), fold Whisper onto `WhisperEmbeddingModule` + `.bin` as `TensorSource` (B31), long-form over `audio/chunking` (B29 merge complete). **No new ASR family port before 11a.** Rules: no new entry points in `capi/audiocpp.h` (F14); no family port before Phase 11a (F13); every ASR result must carry `truncated` honestly (L11, now satisfied); `scripts/sync-deps.sh` at phase boundary (L13).
