...
| B30 *(v6.0)* | `MoonshineKvCache`, `MoonshineStreamingKvCache`, `WhisperKvCache` in `include/engine/models/*/graphs_internal.h` | ~200 | `framework/asr/enc_dec_kv_cache` (+ `DecodeDriver` argmax/suppression shared); W1a/W1b/W2a adopted `enc_dec_kv_cache`; whisper+qwen3_asr+voxtral_realtime+hvisce_asr+nemotron_asr+higgs_audio_stt+fun_asr_nano adopted `argmax_logits` | the three `*_engine_smoke_test` + WER gates green; 6 private argmax impls deleted | Phase 11a (2026-09-14/15) | c59b15a0, a6d02849, d43f754e |
