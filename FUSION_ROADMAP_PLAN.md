...
| B31 *(v6.0)* | `models/whisper/assets.cpp` private `.bin` parser; `models/whisper/graphs.cpp` encoder | ~600 | `assets/whisper_bin_tensor_source` (Part 1 ✅); `WhisperEmbeddingModule` encoder fold (Part 2 ✅) | `whisper_engine_smoke_test`, `asr_e2e_whisper_wer_test`, `whisper_bin_*` units | Phase 11a (2026-09-17) | |
...
v6.0 subtotal: the whole of `src/runtime/` (≈ 95 kLOC measured) plus the engine-side duplicates the first three ports introduced. **As of 2026-08-27: B11–B15 are committed and pushed (9cc5457, fdaa9a5, e3e7eac1, 1be9ac40, a8b7a03b) — Phase 10.5 has retired all five overlapping families. B16–B27 remain for Phase 11.**
