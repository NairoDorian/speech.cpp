...
| `sortformer_diar` **IN PROGRESS — step 1 done (`65ab43c`): first registered gate + cancellation** | the presets are not a knob but a **scheduler**: the arch runs diarization in chunks with an AOSC speaker cache + FIFO (475 LOC of host logic in `arch/sortformer/stream.cpp` plus a two-graph split), driven by a RUN-slot extension whose four operating points set chunk length, right context, FIFO and cache geometry. The engine package is offline-only with one whole-window graph, so this merge is a port, not a wiring change. Both packages are downloadable, and they are **not the same kind of file**: the catalogue's default v2 (`nvidia/diar_streaming_sortformer_4spk-v2`, pinned revision) is a **transcribe.cpp-flavoured GGUF** (`general.architecture` = `sortformer`, 971 tensors) while v1 is an audio.cpp package (`audiocpp` / `sortformer_diar`, 1011 tensors). So unlike families 1 and 2, this arch **can** load a published package and the engine cannot load that one — the retirement has to answer for v2 support, not just for the presets | `sortformer_diar_engine_smoke_test` (landed, step 1: ordering, speaker count, coverage, determinism, cancellation); then the preset round-trip through the typed ext and DER unchanged |
...
| B13–B15 | `src/runtime/arch/{funasr_nano, sensevoice, sortformer}/` | 7,571 | engine counterparts, feature-merged | per-family goldens | Phase 10.5 | |
