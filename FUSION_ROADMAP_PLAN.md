...
### Phase 10.5 — Execute the Phase-10 Verdicts *(v6.0, in progress — 2 of 5 done)*
...
| `voxtral_realtime` **DONE fdaa9a5** | the bake-off row said "cache-aware streaming windows", but the engine already had the sliding-window encoder and decoder caches; what it lacked was the arch's **typed stream extension** — `num_delay_tokens` (hardwired in four places, including a literal `0.480` in the prompt) and `min_decode_interval_ms` — plus cancellation. All three merged; the ArchAdapter now accepts the extension itself, so `transcribe/voxtral_realtime.h` keeps working after the arch is gone | `voxtral_realtime_engine_smoke_test` (WER 2.89855%, 2/69; streamed-vs-offline divergence 0), `voxtral_realtime_delay_test`, `voxtral_realtime_ext_abi_test` through the C ABI. `tests/golden/voxtral_realtime/` still needs the HF reference dump (Track M) |
| `sortformer_diar` **feature-merge DONE (step 2, 2026-08-27); retirement (step 3) next** | the presets are not a knob but a **scheduler**: the arch ran diarization in chunks with an AOSC speaker cache + FIFO (475 LOC of host logic plus a two-graph split), driven by a RUN-slot extension. **Ported**: `src/models/sortformer_diar/streaming.{h,cpp}` (host-for-host), stem/body graph split cached per capacity tier, chunked-by-default for packages that ship an operating point, `stream_preset` + per-field options, the typed ext through the adapter. **Step 1's premise was wrong**: the catalogue's default v2 package is **NVIDIA's own GGUF layout** (`sortformer.encoder.*` KVs, `encoder.layers.N.*` tensors) — a third layout that *neither* parent could open (the arch fails with `missing KV stt.sortformer.max_speakers`). The engine now opens it through a NeMo-layout loader (KV → config + a renaming `TensorSource` view over the HF-named weight loader), so the default package is loadable for the first time. Measured: chunked == whole-window to 1.8e-7; vs the arch on identical weights (package re-keyed into the arch's layout) 0/600 decision flips on the published operating points, max \|Δp\| 7.4e-3 — decision parity, not the arch's 4.5e-4 NeMo tensor tolerance (Track M). Report: `docs/reports/sortformer_diar_engine_port.md`. **Retirement scope**: the parakeet multitalker arch includes the sortformer arch's embedded-diarizer core and parakeet's arch is canonical, so only the standalone family retires (dispatcher entry — which is what routes the v2 package to the engine through the C ABI — `Arch` hooks, the ext initializer's move); the embedded core stays until parakeet migrates | `sortformer_diar_engine_smoke_test` (step 1), `sortformer_diar_streaming_engine_test` + `sortformer_diar_scheduler_test` (step 2), `sortformer_diar_ext_abi_test` through the C ABI (custom tree) — all green |
...
>
> **Status (2026-09-23)**: W1a/W1b retired their arches (B16a/B16b). **W2b is larger than "mostly ASR-layer work"**: the engine Whisper package cannot load the family's real distribution format at all — parent transcribe.cpp publishes every variant as a GGUF under `handy-computer/whisper-*-gguf` (the 2026-08-26 "no downloadable GGUF" finding only held for `audio-cpp/audio.cpp-gguf`). W2b therefore owns (1) GGUF loading via `TensorSource` + the new shared `GgufMetadata`, (2) a graph-agnostic Whisper decoding policy (HF timestamp rules, `_retrieve_segment`, temperature ladder, compression-ratio / logprob / no-speech gates, prompts) with the generic sampling pieces in `framework/asr/`, (3) long-form seek, language detection and translate, and (4) the adapter's run-ext / chunk-trace / capability mapping. The retirement gate exists: four transcribe.cpp C-ABI Whisper tests, registered 2026-09-23 against pinned GGUF and `.bin` models, green on the arch. A premature B16c deletion was parked in `git stash` for failing that bar. Keep any static decode graph GPU-gated until a CPU gate proves it — parent `6c767184` turned it on for CPU and its Whisper decodes garbage. **Later the same day W2b.1 landed**: GGUF + `.bin` loading and the full recipe as a graph-agnostic policy (`src/models/whisper/decoding.*`); the engine matches the arch word-for-word across six modes including 83 s long-form (`whisper_engine_arch_parity_test`). What remains is W2b.2, the C-ABI takeover, then B16c. **Both landed the same day (B16c row in Appendix B)**: the Whisper arch and its parallel `.bin` parser are deleted, the C ABI runs the engine, and the gate held — the four transcribe.cpp C-ABI tests green, 3/69, and a byte-exact C-ABI-vs-engine parity test. Only W2b.3 (batched decode, static step graph) remains, as performance work.
...
| B12 | `src/runtime/arch/voxtral_realtime/` (+ the never-registered `tests/transcribe/voxtral_realtime_real_smoke.cpp`) | 4,996 | `src/models/voxtral_realtime/` with the arch's features merged (61ab725); the family's public ext initializer moves to `src/runtime/transcribe-family-ext.cpp` | `voxtral_realtime_engine_smoke_test`, `voxtral_realtime_delay_test`, `voxtral_realtime_ext_abi_test` | Phase 10.5 (2026-08-26) | fdaa9a5 |
| B13 | `src/runtime/arch/sensevoice/` | 2,601 | `src/models/sense_asr/` with the arch's features merged (`61ab725` + B8/B6 ports); C ABI option-aliasing bug fixed on the way | `asr_e2e_sense_asr_wer_test`, `asr_stream_text_sense_asr_wer_test`, `test_adapter_sniff_dispatch` | Phase 10.5 (2026-08-27) | e3e7eac1 |
| B14 | `src/runtime/arch/funasr_nano/` (+ orphaned `transcribe-kaldi-fbank.cpp` removed from the runtime OBJECT library) | 3,372 | `src/models/fun_asr_nano/` with the arch's features merged (Phase-4 PackedQKV/GateUp already in `13abbd7` + Phase-2 shared K/V via `ScopedWeightShareKey` in `fe1a307`) | `asr_e2e_fun_asr_nano_wer_test`, `test_adapter_sniff_dispatch` | Phase 10.5 (2026-08-27) | 1be9ac40 |
| B15 | `src/runtime/arch/sortformer/` — the **standalone family** only: `sortformer::arch` + its `load`/`init_context`/`run`/`run_validate`/`accepts_ext_kind`, the offline dump forward, `transcribe_sortformer_stream_ext_init` (moves to `transcribe-family-ext.cpp`), `tests/transcribe/sortformer_stream_ext_unit.cpp`. The embedded-diarizer core (`sortformer.h`, `weights.*`, `stream.cpp`, `init_embedded_diarizer` / `fuse_embedded_diar_bn` / `run_diar_streaming_core` / `probs_to_speaker_segments`) is a dependency of the canonical parakeet arch (multitalker) and goes with parakeet's migration | ~700 | `src/models/sortformer_diar/` with the scheduler, presets, typed ext and NeMo layout merged (step 2, 2026-08-27); B15 step 3 retired the standalone dispatcher entry + hooks | `sortformer_diar_streaming_engine_test`, `sortformer_diar_scheduler_test`, `sortformer_diar_ext_abi_test` (re-pointed at the v2 package) | Phase 10.5 (step 3, 2026-08-27) | a8b7a03b |
| B16a | `src/runtime/arch/moonshine/` | 3,506 | `src/models/moonshine/` native engine package (`graphs.cpp`, `assets.cpp`, `runtime.cpp`, `session.cpp`) | `moonshine_engine_smoke_test`, `asr_e2e_wer_test` (1.449% WER = 1/69 edits), `asr_e2e_edits_test` | Phase 11b (2026-09-17) | |
| B16b | `src/runtime/arch/moonshine_streaming/` | 3,630 | `src/models/moonshine_streaming/` native engine package on `StreamingSessionBase`; public stream ext initializer preserved in `src/runtime/transcribe-family-ext.cpp` | `moonshine_streaming_engine_smoke_test`, `asr_stream_text_wer_test` (4.348% WER, divergence 0) | Phase 11b (2026-09-17) | |
| B16c | `src/runtime/arch/whisper/` + `src/runtime/transcribe-bin-loader.{h,cpp}` | 7,045 + 557 | `src/models/whisper/` native engine package (GGUF + `.bin` via `TensorSource`; recipe in `decoding.{h,cpp}`); public Whisper entry points in `src/runtime/transcribe-family-ext.cpp`; run ext translated by the ArchAdapter | `transcribe_whisper_{e2e_smoke,tokenize_parity,bin_e2e_smoke,bin_tokenize_parity}`, `asr_e2e_whisper_wer_test` (3/69), `whisper_c_abi_parity_test` (byte-exact, 6 modes), `whisper_engine{,_gguf}_smoke_test` | Phase 11 W2b (2026-09-23) | |
| B17–B27 | `src/runtime/arch/{voxtral, canary, canary_qwen, cohere, gigaam, granite, granite_nar, medasr, moss}/` | ≈34,000 (41,084 before B16c) | `src/models/<family>/` (new directories) | per-family goldens + WER | Phase 11 | |
...
| B29 *(v6.0)* | `src/runtime/transcribe-vad{,-integrate}.{cpp,h}` | ~500 | `audio/chunking` (`plan_vad_audio_chunks`, `append_chunk_speech_metadata`); arch-level VAD run consolidated into `transcribe.cpp` | `vad_plan_unit`, `vad_merge_unit` deleted; `test_audio_chunking` covers the shared chunker; `asr_e2e_*` with VAD on | Phase 11a (2026-09-14) | c59b15a0 |
| B30 *(v6.0)* | `MoonshineKvCache`, `MoonshineStreamingKvCache`, `WhisperKvCache` in `include/engine/models/*/graphs_internal.h` | ~200 | `framework/asr/enc_dec_kv_cache` (+ `DecodeDriver` argmax/suppression shared); W1a/W1b/W2a adopted `enc_dec_kv_cache`; whisper+qwen3_asr+voxtral_realtime+hviske_asr+nemotron_asr+higgs_audio_stt+fun_asr_nano adopted `argmax_logits` | the three `*_engine_smoke_test` + WER gates green; 7 private argmax impls deleted | Phase 11a (2026-09-14/15) | c59b15a0, a6d02849, d43f754e |
| B31 *(v6.0)* | `models/whisper/assets.cpp` private `.bin` parser; `models/whisper/graphs.cpp` encoder | ~600 | `assets/whisper_bin_tensor_source` (Part 1 ✅); `WhisperEmbeddingModule` encoder fold (Part 2 ✅) | `whisper_engine_smoke_test`, `asr_e2e_whisper_wer_test`, `whisper_bin_*` units | Phase 11a (2026-09-17) | 5318d532 |
...
v6.0 subtotal: the whole of `src/runtime/` (≈ 95 kLOC measured) plus the engine-side duplicates the first three ports introduced. **As of 2026-09-17: B11–B15 and B16a/B16b are retired (9cc5457, fdaa9a5, e3e7eac1, 1be9ac40, a8b7a03b, Phase 11b B16a/b) — 7 arch families retired in favor of native engine packages.**
