...
**speech.cpp is the one local C++/[ggml](https://github.com/ggml-org/ggml) speech runtime behind FreeSpeech.** It serves FreeSpeech desktop (the successor of ZER0 and S2B2S) and FreeSpeech Android. **Both are primary targets.**

It runs every local speech model those apps need:
- STT offline and streaming (committed + tentative partials);
- TTS offline and streaming, voice cloning;
- VAD, diarization, alignment, language identification;
- enhancement, separation, and more.

It does this through **one C ABI (`speech.h`)** and a Rust crate, embedded in-process. Every supported model is verified by numeric gates, and the bundled library contains only the models the app ships.

It is built from three sources:
- [audio.cpp](https://github.com/0xShug0/audio.cpp): the engine, ~100 model families, the model-spec system. speech.cpp is a fork of it and merges it continuously.
- [transcribe.cpp](https://github.com/NairoDorian/transcribe.cpp), the author's optimized fork of handy-computer/transcribe.cpp: the ASR families, the size-aware C ABI, the streaming contract and the parity methodology. Its optimizations carry over to every family that moves onto the engine.
- [CrispASR](https://github.com/CrispStrobe/CrispASR): a reference for further families and verification tooling, mined family by family and never merged.

> [!IMPORTANT]
> **Status: pre-release; the fusion is in progress.**
> - **New here, human or LLM agent? Read [`ARCHITECTURE.md`](ARCHITECTURE.md) first.** It is the generated map of the whole project (directories, sizes, model families, entry points), regenerated by repomix on every commit.
> - The plan, the rules and the current state are in [`PLAN.md`](PLAN.md). Start with its *North star* section.
> - How ports are accepted (never slower or less accurate than the parents): [`docs/benchmarking.md`](docs/benchmarking.md). The vocabulary and metrics (WER, CER, RTF, TTFA, DER, SIM, …) and the standard benchmarks per task are in [`docs/glossary.md`](docs/glossary.md).
> - Decisions: [`docs/LEDGER.md`](docs/LEDGER.md). Lessons: [`LESSONS.md`](LESSONS.md). Agent conventions: [`AGENTS.md`](AGENTS.md).
> - Known-bad items are listed in `PLAN.md` §2.3. Current numbers come from `bash scripts/status.sh`.

### Two build profiles, one codebase

| Profile | What you get | For |
|---|---|---|
| **DEV (full)** | Every model family, plus the HTTP server + WebUI, `audiocpp_cli`, GGUF tools, model manager, tests and benchmarks. Every way to try and test every model. | development, model porting, QA |
| **BUNDLE (embed)** | The embeddable library + `speech.h` (+ Rust crate) with **only the families you select**, down to a single STT or TTS family. No server, WebUI, CLI, tools or tests. | FreeSpeech desktop and Android, and any app embedding speech.cpp |

The BUNDLE profile is being built: named model-set composites, Cargo features, an install tree for Rust `build.rs`, and Android arm64 (`PLAN.md` phase E). Until then, apps embed through the `transcribe.h` C ABI (`SPEECHCPP_ENABLE_UNIFIED_ABI=ON`) or the `client-*` presets (see [Build](#build)).

### From upstream audio.cpp

Upstream's news and releases; speech.cpp merges them:

- GGUF packages: [huggingface.co/audio-cpp/audio.cpp-gguf](https://huggingface.co/audio-cpp/audio.cpp-gguf) (ModelScope mirror: [HereIsMark/audio.cpp-gguf](https://www.modelscope.cn/models/HereIsMark/audio.cpp-gguf)); Colab: [audio.cpp notebook](https://colab.research.google.com/github/0xShug0/audio.cpp/blob/main/Notebooks/colab_audio_cpp.ipynb).
...
- [PLAN.md: north star, rules, plan](PLAN.md)
- [News](#news)
...
- **C ABI.** Today: the `transcribe.h` C API (from transcribe.cpp, `SPEECHCPP_ENABLE_UNIFIED_ABI=ON`) reaching engine families through an adapter. Target: `speech.h` as the only first-party C ABI, with a Rust crate first, then Python and TypeScript (see [Bindings](#bindings)).
...
> [!TIP]
> **Contribution focus:** follow the phase order in [`PLAN.md`](PLAN.md): stabilize and sync the parents (S0), hygiene gates (S1), the shared ASR layer (S2), `speech.h` (S3), and the BUNDLE profile (E). Read the *North star* rules first; every change must keep both FreeSpeech desktop and Android working.
>
...
These 20 speech families come from transcribe.cpp and are being moved onto the speech.cpp engine family by family: verdict against the original arch, then retirement of the arch. Their numerically verified GGUFs are published under [`handy-computer`](https://huggingface.co/handy-computer). **The migration state of each family is in [`docs/LEDGER.md`](docs/LEDGER.md) §2.** The model cards live in transcribe.cpp:

| Family | Variants | Docs |
| --- | --- | --- |
| Parakeet | 11 variants: TDT, RNN-T, CTC, TDT+CTC (110M–1.1B), incl. German `parakeet-primeline` | [parakeet.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/parakeet.md) |
| Canary | `canary-1b`, `canary-1b-v2`, `canary-1b-flash`, `canary-180m-flash` | [canary.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/canary.md) |
| Canary-Qwen | `canary-qwen-2.5b` (FastConformer + Qwen3-1.7B SALM) | [canary-qwen-2.5b.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/canary-qwen-2.5b.md) |
| Whisper | 12 variants (`tiny` through `large-v3-turbo`, plus `.en` siblings) | [whisper.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/whisper.md) |
| GigaAM | `gigaam-v3-{e2e-rnnt,e2e-ctc,rnnt,ctc}` | [gigaam.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/gigaam.md) |
| Moonshine | `moonshine-tiny`, `moonshine-base` | [moonshine.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/moonshine.md) |
| Moonshine Streaming | `moonshine-streaming-{tiny,small,medium}` | [moonshine-streaming.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/moonshine-streaming.md) |
| Qwen3-ASR | `qwen3-asr-0.6b`, `qwen3-asr-1.7b` | [qwen3-asr.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/qwen3-asr.md) |
| Cohere Transcribe | `cohere-transcribe-03-2026` | [cohere-transcribe-03-2026.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/cohere-transcribe-03-2026.md) |
| SenseVoice | `sensevoice-small` | [sensevoice-small.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/sensevoice-small.md) |
| FunASR Nano | `fun-asr-nano-2512`, `fun-asr-mlt-nano-2512` | [fun-asr-nano.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/fun-asr-nano.md) |
| Nemotron Speech Streaming | `nemotron-speech-streaming-en-0.6b` | [nemotron-speech-streaming-en-0.6b.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/nemotron-speech-streaming-en-0.6b.md) |
| Nemotron 3.5 ASR Streaming | `nemotron-3.5-asr-streaming-0.6b` (multilingual, 40 locales) | [nemotron-3.5-asr-streaming-0.6b.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/nemotron-3.5-asr-streaming-0.6b.md) |
| Multitalker Parakeet Streaming | `multitalker-parakeet-streaming-0.6b-v1` (single-speaker ASR path only) | [multitalker-parakeet-streaming-0.6b-v1.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/multitalker-parakeet-streaming-0.6b-v1.md) |
| Granite Speech 4 / 4.1 | `granite-4.0-1b-speech`, `granite-speech-4.1-2b{,-plus,-nar}` | [granite-speech.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/granite-speech.md) |
| Voxtral | `voxtral-mini-3b-2507`, `voxtral-small-24b-2507` (audio-LLM; transcription + translation) | [voxtral.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/voxtral.md) |
| Voxtral Realtime | `voxtral-mini-4b-realtime-2602` (streaming audio-LLM) | [voxtral-realtime.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/voxtral-realtime.md) |
| MedASR | `medasr` (Conformer + CTC, English medical-dictation, gated) | [medasr.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/medasr.md) |
| MOSS Transcribe-Diarize | `moss-transcribe-diarize` (audio-LLM; English + Chinese ASR with inline speaker diarization) | [moss-transcribe-diarize.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/moss-transcribe-diarize.md) |
| Sortformer | `diar_streaming_sortformer_4spk-v2.1` (streaming speaker diarizer, up to 4 speakers; no transcription) | [diar_streaming_sortformer_4spk-v2.1.md](https://github.com/NairoDorian/transcribe.cpp/blob/main/docs/models/diar_streaming_sortformer_4spk-v2.1.md) |

Per-variant model cards for these families live in [transcribe.cpp `docs/models/`](https://github.com/NairoDorian/transcribe.cpp/tree/main/docs/models). speech.cpp's own [`docs/models/`](docs/models/) covers the audio.cpp families.

### Overlap Resolution

Some STT families exist in both parents. The **engine package wins**: the
transcribe.cpp arch implementations of qwen3_asr, voxtral_realtime, sensevoice,
funasr_nano, moonshine, moonshine_streaming and whisper are retired, and the C ABI
reaches the engine through the `ArchAdapter` bridge until `src/runtime/` is deleted.
Remaining arch families and their state are listed in
[`docs/LEDGER.md`](docs/LEDGER.md) §2; the plan is [`PLAN.md`](PLAN.md).
...
A stopgap until the BUNDLE profile (`PLAN.md` phase E) replaces it. It ships the legacy `audiocpp` C ABI plus the CLI and GGUF tool; the BUNDLE profile will be library-only, with `speech.h`/`transcribe.h`, model-set composites and Cargo features. For client-side apps embedding speech without the HTTP server, WebUI assets or unused models:
...
| Surface | State |
| --- | --- |
| `include/transcribe/transcribe.h` | **The C ABI apps can embed today** (`SPEECHCPP_ENABLE_UNIFIED_ABI=ON`). It becomes a compatibility shim over `speech.h`, so the `transcribe-cpp` Rust crate used by ZER0 and Android_FreeSpeech keeps working. |
| `include/speech/speech.h` | **Target first-party ABI** (`PLAN.md` S3). Not written yet. |
| `capi/include/audiocpp.h` | Legacy speech.cpp C ABI. Frozen, and retired after `speech.h` (V7-D1). |
| `include/audiocpp.h` | Upstream audio.cpp's own C ABI, carried unchanged. |
| Rust crate (`speech-sys` / `speech`) | Planned first (`PLAN.md` phase 13). Model-set Cargo features; builds from source or links a BUNDLE install tree. |
| Python | `bindings/python` (configuration only so far); generated bindings come after Rust. |
| TypeScript, Swift | Planned after Rust and Python; transcribe.cpp's generator will be ported. |
| WebUI | DEV-profile tool, embedded in `audiocpp_server` ([webui/README.md](webui/README.md)). |

See [`docs/bindings.md`](docs/bindings.md) for how transcribe.cpp generates bindings from the header, which is the model the `speech.h` bindings will follow.
...
For the STT model-family smoke-test, numerical-validation and benchmark pattern, see [`docs/model-family-testing.md`](docs/model-family-testing.md).

**Test tiers** (`AGENTS.md`):
- **quick** (default): every model-backed verdict or parity test loads its model and runs **one short clip**. The whole tier takes ~4 min on CPU.
- **full** (opt-in, `-DSPEECHCPP_FULL_ASR_TESTS=ON`, label `full`): the multi-clip corpus, batched passes and all modes.

```powershell
# suite of record (framework + C ABI, no model families linked)
& "C:\Program Files\CMake\bin\ctest.exe" --test-dir build-cpu-core -C Release -j 8
# model-backed ASR gates: WER, verdicts, parity (quick tier)
& "C:\Program Files\CMake\bin\ctest.exe" --test-dir build-cpu-asr-abi -C Release -L quick -j 2
```

Test models are fetched, sha256-pinned, with `uv run scripts/fetch_asr_test_model.py`. A test whose model is absent reports SKIP, never PASS.
...
```
include/engine/            Engine C++ API (sessions, registry, framework, model packages)
include/transcribe/        transcribe.h: the C ABI apps embed today (becomes a shim over speech.h)
include/audiocpp.h         upstream audio.cpp's C ABI (carried unchanged)
capi/                      legacy speech.cpp audiocpp.h C ABI (frozen; retired after speech.h)
src/framework/             shared engine layers: runtime, assets, audio, asr, codecs, tokenizers, ...
src/models/                core model families (engine packages)
src/community_models/      community model families (engine packages)
src/runtime/               transcribe.cpp runtime + remaining arch families (deleted in phase 11c)
model_specs/               model package specifications (JSON)
app/                       DEV tools: cli, server (+ WebUI), gguf tool, model manager, workflow
webui/                     WebUI sources (DEV profile)
external/ggml/             vendored ggml, generated: pin in external/ggml/UPSTREAM + patches/ggml/
patches/ggml/              downstream ggml patches (scripts/sync-ggml.sh)
scripts/                   sync-deps.sh, sync-ggml.sh, status.sh, fetch_asr_test_model.py, converters
tests/                     unit, parity, verdict and WER gates (quick / full tiers)
assets/                    bundled assets; assets/asr_validation/{quick,librispeech} fixtures
bindings/                  python (config); rust, ts and swift planned (PLAN phase 13)
docs/                      LEDGER.md, reports/, porting/, upstream/ triage ledgers, archive/
PLAN.md LESSONS.md AGENTS.md   the plan (north star + rules), lessons, agent conventions
```
...
> [!TIP]
> **Contribution focus:** see [`PLAN.md`](PLAN.md) (phase order and the *North star* rules). DEV tooling (WebUI, server, CLI) is welcome as long as the library never depends on it.
>
...
Repository discipline for humans and agents is in [`AGENTS.md`](AGENTS.md): `uv run` for all Python, per-family reference envs under `scripts/envs/<family>/`, the C ABI exception rules (every public entry point routes through `api_guard_*` or is nothrow by construction; "non-OK ⇒ `*out == NULL`, nothing leaked"), the upstream sync rules and the test tiers. transcribe.cpp's pinned clang-format script is not vendored yet (`PLAN.md` S1.5); format by hand to the style of the file you edit.

---

## License

speech.cpp is licensed under the **Apache License 2.0** (see [LICENSE](LICENSE)), inherited from its fork base audio.cpp. It incorporates MIT-licensed code, whose notices must be preserved:

- **audio.cpp** ([0xShug0/audio.cpp](https://github.com/0xShug0/audio.cpp), Apache-2.0): the engine and model families.
- **transcribe.cpp** ([handy-computer/transcribe.cpp](https://github.com/handy-computer/transcribe.cpp), via [NairoDorian/transcribe.cpp](https://github.com/NairoDorian/transcribe.cpp); MIT): the runtime, C ABI and ASR families.
- **CrispASR** ([CrispStrobe/CrispASR](https://github.com/CrispStrobe/CrispASR), MIT): referenced; any code taken from it is attributed per file.

Vendored third-party components include ggml (MIT), miniz (MIT), sentencepiece, llama_tokenizer, cJSON, libyaml and dr_wav. A consolidated notices file (transcribe.cpp's `THIRD-PARTY-LICENSES.md` plus audio.cpp's components) is **still to be added**; it is tracked in `PLAN.md` S1.6. Licence sign-off before 1.0 is ledger item MR-4. Model weights carry their own licences; non-commercial models are opt-in only (V7-D5).
