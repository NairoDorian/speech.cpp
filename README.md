...
- **C ABI.** A public C API (from transcribe.cpp) enabling Python, TypeScript, Rust, and Swift bindings alongside the native C++ API.
...
Model weights keep the license of their original release, which is separate from speech.cpp's own code license (Apache-2.0 from audio.cpp; code derived from transcribe.cpp is MIT). [docs/model_licenses.md](docs/model_licenses.md) lists that license per family, and whether it allows commercial use.
