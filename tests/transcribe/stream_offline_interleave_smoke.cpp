...
bool run_offline(transcribe_session * s, const std::vector<float> & pcm, std::string & out) {
    const transcribe_status st = transcribe_run(s, pcm.data(), static_cast<int>(pcm.size()), nullptr);
    if (st != TRANSCRIBE_OK) {
        std::fprintf(stderr, "   offline run returned %s\n", transcribe_status_string(st));
        return false;
    }
    out = text_of(s);
    return true;
...
// speech.cpp: say what differed, not only that something did.
bool same_text(const std::string & got, const std::string & want, const char * what) {
    if (got == want) {
        return true;
    }
...
int test_model(const char * label, const char * path, const std::vector<float> & pcm) {
    ...
    std::string t;
    CHECK(run_stream(s, pcm, -1, t) && same_text(t, ref_stream, "stream 1"));
    CHECK(run_offline(s, pcm, t) && same_text(t, ref_offline, "offline 1"));
    CHECK(run_stream(s, pcm, -1, t) && same_text(t, ref_stream, "stream 2"));
    CHECK(run_offline(s, pcm, t) && same_text(t, ref_offline, "offline 2"));
    CHECK(run_stream(s, pcm, /*reset_at_feed=*/5, t) && same_text(t, ref_stream, "stream 3 (reset)"));
    CHECK(run_offline(s, pcm, t) && same_text(t, ref_offline, "offline 3"));
    CHECK(run_stream(s, pcm, -1, t) && same_text(t, ref_stream, "stream 4"));
    transcribe_close(s);
