...
inline void portable_setenv(const char * name, const char * val) {
#if defined(_WIN32)
    _putenv_s(name, val);
#else
    ::setenv(name, val, 1);
...
int main() {
    const char * gguf_env = std::getenv("TRANSCRIBE_SORTFORMER_GGUF");
    if (gguf_env == nullptr || gguf_env[0] == '\0') {
        std::fprintf(stderr, "sortformer_stream_ext_unit: skipped (set TRANSCRIBE_SORTFORMER_GGUF to run)\n");
        return 77;
    }
    ...
    portable_unsetenv("TRANSCRIBE_SORTFORMER_STREAM_PRESET");
    ...
    portable_setenv("TRANSCRIBE_SORTFORMER_STREAM_PRESET", "very_high_latency");
