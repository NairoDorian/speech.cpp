# Releasing speech.cpp prebuilt binaries

> Ported from audio.cpp `c79e588` ("ci: add tag-driven prebuilt release
> pipeline", #286) and rebranded for this fork: release artifacts are named
> `speech-<tag>-...` rather than `audio-<tag>-...`. The build targets
> (`audiocpp_cli` / `audiocpp_server` / `audiocpp_gguf`) and the
> `windows-*-release` presets in `scripts/build_windows.ps1` are unchanged, so
> the pipeline runs against this tree as-is.
...
2. On a real NVIDIA GPU machine, run e.g.
   `audiocpp_cli.exe --backend cuda ...` (the `speech_cli` / `speechcpp_cli`
   CMake ALIAS targets build the same binary) and confirm the log reports a CUDA
