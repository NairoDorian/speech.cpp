...
// position/slot, and its own row in the batched attention mask.
...
public:
    DecodeGraphBatched(
        ...
        debug::trace_log_scalar("higgs_audio_stt.text_decoder.decode_batched_cache_steps",
                                static_cast<double>(cache_steps_));
        debug::trace_log_scalar("higgs_audio_stt.text_decoder.decode_batched_n_seqs",
                                static_cast<double>(n_seqs_));
    }
