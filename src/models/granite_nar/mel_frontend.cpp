// engine/models/granite_nar/mel_frontend.cpp - the arch's log-mel frontend
// (src/runtime/transcribe-mel.cpp, MelFrontend), ported verbatim so the
// granite_nar engine package sees the exact features the arch saw. Copied
// from src/models/canary_qwen/mel_frontend.cpp (itself a verbatim port of the
// same MelFrontend); only the namespace and class names differ. See
// mel_frontend.h for the path granite_nar takes and why
// engine::audio::MelExtractor is not used.
//
// Constants / ordering (every MelFrontend mode is kept, granite_nar uses the
// per_utterance one):
//   * pre-emphasis alpha (0 for granite_nar), reflect pad by n_fft/2
//   * STFT fp64 radix-2 for pow2 n_fft (granite_nar: 512), fp32 mixed radix
//     otherwise; Hann zero-padded to n_fft (or the GGUF's window, centred)
//   * power = re^2 + im^2; Slaney filterbank (or the GGUF's)
//   * per_utterance / global: log10(max(x, 1e-10)), clamp to max - 8,
//     (x + 4) / 4, trailing centre-pad frame dropped
//   * per_feature: log(x + 2^-24), unbiased (x - mean) / (std + 1e-5)
//
...
GraniteMelFrontend::GraniteMelFrontend(const GraniteMelConfig &cfg) : cfg_(cfg) {
...
int GraniteMelFrontend::n_frames_for(size_t n_samples) const {
...
bool GraniteMelFrontend::compute(const float *pcm, size_t n_samples, std::vector<float> &out_mel,
  ...
  // ---- 3. Per-feature normalize, unbiased (NeMo; not granite_nar) ----
