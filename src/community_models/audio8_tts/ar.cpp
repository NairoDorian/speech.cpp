...
std::unique_ptr<FalconStepPlan> build_falcon_step_plan(
    ...
    for (int64_t li = 0; li < n_layer; ++li) {
        ...
        // K = 1: keep only the final recurrent state, the pre-0.24.0 layout
        // (y values, then one state) the views below slice. speech.cpp's ggml
        // 0.24.0 takes K explicitly (ggml-org llama/26623); audio.cpp's does not.
        ggml_tensor * scan = ggml_ssm_scan(ctx, ssm_t, x4, dt3, A_t, B4, C4, ids, 1);
...
SlowForwardOutput falcon_forward_step(
    ...
    for (int64_t li = 0; li < n_layer; ++li) {
        ...
        // (y values, then one state) the views below slice. speech.cpp's ggml
        // 0.24.0 takes K explicitly (ggml-org llama/26623); audio.cpp's does not.
        ggml_tensor * scan = ggml_ssm_scan(ctx.get(), ssm_t, x4, dt3, A_t, B4, C4, ids, 1);
