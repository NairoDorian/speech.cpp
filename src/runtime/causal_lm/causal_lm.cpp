...
int pick_kv_cache_context(int needed, int model_max) {
    return engine::modules::transformers::pick_kv_cache_context(needed, model_max);
}
...
void fill_prefill_chunk_mask(ggml_fp16_t * dst, int max_n_kv, int T_chunk, int n_past) {
    ...
    engine::modules::transformers::fill_prefill_chunk_mask(dst, max_n_kv, T_chunk, n_past);
}

int prefill_chunk_size() {
    return engine::modules::transformers::prefill_chunk_size();
}
