...
ggml_tensor * shaw_block_attn(ggml_context *          ctx,
    ...
    // LinearModule weights are logical [out, in] = ggml {ne[1], ne[0]}. Until
    // 2026-09-24 these were wrapped {ne[0], ne[1]}: fine for the square q/out
    // projections of the shipped checkpoints, a shape-check throw on the fused
    // (in -> 2*inner) attn_kv - the granite arches could not encode.
    ew.attn_q = {
        engine::core::wrap_tensor(w.attn_q_w, engine::core::TensorShape::from_dims({w.attn_q_w->ne[1], w.attn_q_w->ne[0]}), w.attn_q_w->type),
        std::nullopt
    };
    ew.attn_kv = {
        engine::core::wrap_tensor(w.attn_kv_w, engine::core::TensorShape::from_dims({w.attn_kv_w->ne[1], w.attn_kv_w->ne[0]}), w.attn_kv_w->type),
        std::nullopt
    };
    ...
    ew.attn_out = {
        engine::core::wrap_tensor(w.attn_out_w, engine::core::TensorShape::from_dims({w.attn_out_w->ne[1], w.attn_out_w->ne[0]}), w.attn_out_w->type),
